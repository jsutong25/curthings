...
if (isset($_POST['change-password'])) {
  ...
  if ($currentpass == $data['password']) {
    if ($password !== $cpassword) {
      echo "<script>alert('Password not matched.');window.location.href='profile.php';</script>";
    } else {
    ...
    echo "<script>alert('Your current password is incorrect.');window.location.href='profile.php';</script>";
  }
}
...
<html lang="en">
    ...
    <body>
        ...
        <main>
            <div class=" flex flex-row max-container h-fit">

                <!-- LEFT SIDE -->
                <div class="flex flex-col w-fit">
                    <ul class="p-5">
                        <a href="profile.php" class="font-subtitle regular-18"><li class="my-2 underline bold-18">Profile</li></a>
                        <li class="my-2"><div class="bg-green-main w-full h-0.5"></div></li>
                        <a href="profile_bookingHistory.php" class="font-subtitle regular-18"><li class="my-2">Booking History</li></a>
                    </ul>
                </div>
                ...
                <!-- User info -->
                <div class="flex flex-col lg:flex-row mx-auto gap-8">
                  <div>
                    <?php
                    $query = "SELECT name, email FROM user_info WHERE user_id ='$user_id'";
                    $result = mysqli_query($conn, $query);
                    ?>
                    <div class="flex flex-col md:flex-col mt-5 px-8 gap-5">
                    <?php if (mysqli_num_rows($result) > 0) {
                      while ($data = mysqli_fetch_assoc($result)) { ?>
                      <div class="flex flex-row">
                        <h4 class="font-subtitle bold-18 mr-2">Name:</h4><p class="font-subtitle"><?php echo $data[
                          'name'
                        ]; ?></p>
                      </div>
                      
                      <div class="flex flex-row">
                        <h4 class="font-subtitle bold-18 mr-2">Email:</h4><p class="font-subtitle"><?php echo $data[
                          'email'
                        ]; ?></p>
                      </div>
                      <?php }
                  ...
                  <div class="mt-5 px-8 py-5 border-2 border-green-nav w-fit mx-auto">
                      <div class="flex flex-col justify-center">
                          <h2 class="bold-20 font-subtitle">Change Password</h2>
                          <?php if (count($errors) > 0) { ?>
                              ...
                              <?php } ?>
                          <form action="" method="POST" class="mt-5 px-5 flex flex-col">

                              <label for="currentpass" class="font-subtitle regular-18">Current Password:</label>
                              <input type="password" name="currentpass" autocomplete="off" required placeholder="Enter current password" class="border-0 border-l-4 border-green-main py-2 mb-5 bg-gray-100 rounded-none w-96">
                              
                              <label for="password" class="font-subtitle regular-18">Create new password:</label>
                              <input type="password" name="password" autocomplete="off" required placeholder="Enter new password" class="border-0 border-l-4 border-green-main py-2 mb-5 bg-gray-100 rounded-none w-96">
                              
                              <label for="cpassword" class="font-subtitle regular-18">Confirm password:</label>
                              <input type="password" name="cpassword" autocomplete="off" required placeholder="Confirm your password" class="border-0 border-l-4 border-green-main py-2 mb-5 bg-gray-100 rounded-none w-96">
                              
                              <input type="submit" name="change-password" class="btn_dark_green font-subtitle mt-5" value="Change password" onclick="return confirm('Are you sure you want to change password?');">
                          </form>
                      </div>
                  </div>
                </div>
