...
<body>
    ...
    <main>
      <div class="max-container mt-5">
        <h3 class="font-subtitle bold-32 mb-2 mx-6 lg:mx-0">Products</h3>
        <div class="w-full h-1 bg-green-main rounded-full mb-5"></div>

        <!-- Add the product-container class to the container of product cards -->
        <div class="flex flex-wrap justify-evenly">
          <?php
          ($select_product = mysqli_query($conn, "SELECT * FROM `products`")) or
            die('query failed');
          if (mysqli_num_rows($select_product) > 0) {
            while ($fetch_product = mysqli_fetch_assoc($select_product)) { ?>
              <!-- Add the product-card class to each product card -->
              <a class="flex flex-col group bg-white border shadow-sm rounded-xl overflow-hidden hover:shadow-lg transition m-2 my-5" href="product-view.php?product=<?php echo $fetch_product[
                'product_id'
              ]; ?>">
                <div class="relative rounded-t-xl overflow-hidden">
                  <?php
                  $product_image = reset(
                    explode(',', $fetch_product['product_image'])
                  );
                  echo '<img class="group-hover:scale-105 transition-transform duration-500 ease-in-out rounded-t-xl" src="uploaded_img/' .
                    $product_image .
                    '" alt="" style="width:250px;height:300px;" />';
                  ?>
                </div>
                <div class="p-4 md:p-5">
                  <h3 class="bold-18 font-subtitle text-gray-800 dark:text-white">
