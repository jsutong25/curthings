...
function fetch_data($db, $tableName, $columns)
{
  if (empty($db)) {
    ...
    $query =
      "SELECT " . $columnName . " FROM $tableName" . " ORDER BY user_id DESC";
...
$sql_quotationCount =
  'SELECT COUNT(quotation_id) AS quotationCount FROM quotation WHERE quotation_status = "0"';
$result_quotationCount = $conn->query($sql_quotationCount);

if (isset($_SESSION['user_id'])) {
  if ($result_quotationCount->num_rows > 0) {
    $row = $result_quotationCount->fetch_assoc();
...
$sql_projectCount =
  'SELECT COUNT(booking_id) AS projectCount FROM bookings WHERE booking_status = "0"';
$result_projectCount = $conn->query($sql_projectCount);

if (isset($_SESSION['user_id'])) {
  if ($result_projectCount->num_rows > 0) {
    $row = $result_projectCount->fetch_assoc();
...
$sql_productCount = 'SELECT COUNT(product_id) AS productCount FROM products';
$result_productCount = $conn->query($sql_productCount);

if (isset($_SESSION['user_id'])) {
  if ($result_productCount->num_rows > 0) {
    $row = $result_productCount->fetch_assoc();
   ...
   <body class="">
        ...
        <aside id="default-sidebar" class="hidden sm:inline fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
            <div class="h-full px-3 py-4 overflow-y-auto bg-green-main dark:bg-gray-800">
                ...
                <ul class="text-center space-y-2 mt-5">
                    <li>
                        <a href="adminIndex.php" class="sidebar-a group bg-gray-800">
          ...
          <img src="../public/assets/logo.png" alt="" width=40 height=20 class="bg-gray-800 rounded-full p-1.5">
            ...
            <aside class="sm:disabled sm:hidden">
                ...
                <div class="flex flex-col justify-center items-center gap-y-2 py-4">
                    ...
                    <div class="hs-tooltip inline-block [--placement:right]">
                    <button type="button" class="hs-tooltip-toggle w-[2.375rem] h-[2.375rem] inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-full border border-transparent text-gray-500 hover:bg-gray-800">
                        <a href="../public/login.php?logout=<?php echo $user_id; ?>" name="logout" onclick="return confirm('are your sure you want to logout?');">
                        <svg class="sidebar-svg" aria-hidden="true" fill="none" viewBox="0 0 22 21" xmlns="http://www.w3.org/2000/svg"><path d="m17 16 4-4m0 0-4-4m4 4H7m6 4v1a3 3 0 0 1-3 3H6a3 3 0 0 1-3-3V7a3 3 0 0 1 3-3h4a3 3 0 0 1 3 3v1" stroke="#f3f4f6" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="stroke-374151"></path></svg>
                            <span class="font-subtitle hs-tooltip-content hs-tooltip-shown:opacity-100 hs-tooltip-shown:visible opacity-0 inline-block absolute invisible z-20 py-1.5 px-2.5 bg-gray-900 text-xs text-white rounded-lg whitespace-nowrap dark:bg-neutral-700" role="tooltip">
                            Logout
                            </span>
        ...
        <main>
            <div class="p-4 sm:ml-64">
                <h3 class="bold-32 font-subtitle mb-5">Dashboard</h3>

                <div class="mb-5 mt-8 pt-5 flex flex-col lg:flex-row flex-wrap justify-evenly gap-2">
                    <div class="flex-shrink-0 w-60 sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/4">
                      <a href="adminQuotation.php">
                        <div class="bg-red-600 hover:bg-red-700 hover:cursor-pointer hover:shadow-lg text-white px-5 py-10 text-center rounded-xl">
                          <h5 class="font-subtitle">Pending Quotations</h5>
                          <span class="bold-32">( <?php echo $quotationCount; ?> )</span>
                        </div>
                      </a>
                    </div>
                    ...
                    <div class="flex-shrink-0 w-60 sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/4">
                      <a href="adminBooking.php">
                        <div class="bg-yellow-600 hover:bg-yellow-700 hover:cursor-pointer hover:shadow-lg text-white px-5 py-10 text-center rounded-xl">
                          <h5 class="font-subtitle">Pending Projects</h5>
                          <span class="bold-32">( <?php echo $projectCount; ?> )</span>
                        </div>
                      </a>
                    </div>
                    ...
                    <div class="flex-shrink-0 w-60 sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/4">
                      <a href="adminProducts.php">
                        <div class="bg-green-main hover:bg-green-nav hover:cursor-pointer hover:shadow-lg text-white px-5 py-10 text-center rounded-xl">
                          <h5 class="font-subtitle">All Products</h5>
                          <span class="bold-32">( <?php echo $productCount; ?> )</span>
                        </div>
                      </a>
                    </div>
                </div>
                ...
                <div class="border-2 border-gray-300 shadow-lg p-5 mt-8">  
                    <h4 class="bold-20 font-subtitle mb-5">Recent Projects Images</h4>
                    <div class="mt-8">
                        <?php
                        $query =
                        ...
                        <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                            ...
                            <img class="py-2 mx-2" src="../public/recent_projects_img/<?php echo $data[
                              'recent_proj_img'
                            ]; ?>" alt="" width=200 h=200> 
                            </td>
                            <td scope="col" class="px-6 py-3 text-center gap-2 flex md:inline-block">
                              <a href="edit-recent-project.php?edit=<?= $data[
                                'recent_projects_id'
                              ...
                              <a href="adminProducts.php?delete=<?= $data[
                                'recent_projects_id'
                              ] ?>" class="bg-red-600 font-subtitle px-5 py-2 rounded-lg text-white font-bold tracking-wider hover:shadow-lg hover:bg-red-700 hover:cursor-pointer" onclick="return confirm(`Are you sure you want to remove image '<?php echo $data[
  'recent_projects_id'
]; ?>'`);">Remove</a>
                            </td>
                        <tr>
