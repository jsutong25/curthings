...
$columns = ['recent_projects_id', 'recent_proj_img'];

if (isset($_POST['submit'])) {
  $recent_proj_img = $_FILES['recent_proj_img']['name'];
  $recent_proj_img_tmp_name = $_FILES['recent_proj_img']['tmp_name'];
  $recent_proj_img_folder = '../public/recent_projects_img/' . $recent_proj_img;

  if (empty($recent_proj_img)) {
    echo "<script>alert('Please upload an image.')</script>";
  } else {
    $insert_query = mysqli_query(
      $conn,
      "update recent_projects set recent_proj_img ='$recent_proj_img' where recent_projects_id = '$id'"
    );
    if ($insert_query > 0) {
      move_uploaded_file($recent_proj_img_tmp_name, $recent_proj_img_folder);
      echo "<script>alert('Image updated.');window.location.href='adminIndex.php';</script>";
    } else {
      echo "<script>alert('Error 3.');window.location.href='adminIndex.php';</script>";
    }
  }
}
   ...
   <body class="">
      ...
      <main>
        <div class="p-4 sm:ml-64">
            <div class="mb-5">
                <a href="adminIndex.php" class="text-green-nav flex flex-row font-subtitle bold-18 hover:text-gray-800 hover:underline">
        ...
        $select = mysqli_query(
          $conn,
          "SELECT * FROM recent_projects WHERE recent_projects_id = '$id'"
        );
        while ($row = mysqli_fetch_assoc($select)) { ?>
            ...
            <div class="border-2 border-gray-300">
                ...
                <div class="">
                    <label for="recent_proj_img" class="font-subtitle regular-18">Product Image</label>
                    <input type="file" name="recent_proj_img" class="block w-full font-subtitle text-sm text-gray-500
                ...
                <div>
                    <input type="submit" value="Update Image" name="submit" class="bg-red-700 shadow-md text-white font-subtitle bold-18 py-1 cursor-pointer hover:bg-red-900 mt-2" onclick="return confirm('Are you sure you want to update product');">
                </div>

                <div class="w-full h-2 bg-gray-700 rounded-full my-5"></div>

                <div class="">
                    <a href="adminIndex.php" class="flex flex-row gap-2 items-center rounded-lg justify-center bg-gray-800 shadow-md text-white font-subtitle bold-18 py-1 px-2 cursor-pointer hover:bg-gray-900 hover:shadow-md mt-2">
