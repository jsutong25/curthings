...
if (isset($_POST['submit'])) {
  ...
  mysqli_stmt_bind_param($stmt, "si", $quotation_status, $id);
  $insert_query = mysqli_stmt_execute($stmt);
  mysqli_stmt_close($stmt);

  if ($insert_query) {
    echo "<script>alert('Status updated.'); window.location.replace('adminQuotation.php');</script>";
  } else {
    echo "<script>alert('Error: " .
      mysqli_error($conn) .
      "'); window.location.href='adminQuotation.php';</script>";
  }
}
      ...
      <aside id="default-sidebar" class="hidden sm:inline fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
        <div class="h-full px-3 py-4 overflow-y-auto bg-green-main dark:bg-gray-800">
            ...
            <ul class="text-center space-y-2 mt-5">
                ...
                <li>
                    <a href="adminQuotation.php" class="sidebar-a group bg-gray-800">
                    <svg class="sidebar-svg" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0,0,256,256">
          ...
          <h2 class="bold-32 font-subtitle mb-5">Edit Status for ID: '<?php echo $id; ?>'</h2>
            ...
            <?php if (mysqli_num_rows($result) > 0) {
              ...
              while ($data = mysqli_fetch_assoc($result)) { ?>
                ...
                <?php $quotation_status_ref = $data['quotation_status']; ?>
                <label for="quotation_status" class="font-subtitle">Change Status:</label>
                <select name="quotation_status" id="quotation_status" class="h-10 px-4 font-subtitle">
                    <option value="<?php echo $quotation_status_ref; ?>">Current status: <?php echo $quotation_status_ref; ?></option>
            ...
            <?php }
                ...
                <div>
                    <input type="submit" value="Update status" name="submit" class="bg-red-700 shadow-md text-white font-subtitle bold-18 py-1 cursor-pointer hover:bg-red-900 mt-2" onclick="return confirm('Are you sure you want to update status');">
                </div>

                <div class="w-full h-2 bg-gray-700 rounded-full my-5"></div>

                <div class="">
                    <a href="adminQuotation.php" class="flex flex-row gap-2 items-center rounded-lg justify-center bg-gray-800 shadow-md text-white font-subtitle bold-18 py-1 px-2 cursor-pointer hover:bg-gray-900 hover:shadow-md mt-2">
