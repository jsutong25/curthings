...
if (isset($_POST['submit'])) {
  ...
  $stmt = mysqli_prepare(
    $conn,
    "UPDATE user_info SET role = ? WHERE user_id = ?"
  );
  ...
  if ($insert_query) {
    echo "<script>alert('Role updated.'); window.location.replace('adminUsers.php');</script>";
  } else {
   ...
   <body>
      ...
      <main>
          <div class="p-4 sm:ml-64">
            ...
            <div class="mb-5">
                <a href="adminUsers.php" class="text-green-nav flex flex-row font-subtitle bold-18 hover:text-gray-800 hover:underline">
          ...
          <h2 class="bold-32 font-subtitle mb-5">Edit Status for ID: '<?php echo $id; ?>'</h2>
            ...
            <?php if (mysqli_num_rows($result) > 0) {
              ...
              while ($data = mysqli_fetch_assoc($result)) { ?>
                <label for="quotation_status" class="font-subtitle">Change Role: <span class="font-bold">(Current Role is - <?php echo $data[
                  'role'
                ]; ?> ) </span></label>
                <select name="role" id="role" class="h-10">
            ...
            <?php }
                ...
                <div>
                    <input type="submit" value="Update role" name="submit" class="bg-red-700 shadow-md text-white font-subtitle bold-18 py-1 cursor-pointer hover:bg-red-900 mt-2" onclick="return confirm('Are you sure you want to update role of user?');">
                </div>

                <div class="w-full h-2 bg-gray-700 rounded-full my-5"></div>

                <div class="">
                    <a href="adminUsers.php" class="flex flex-row gap-2 items-center rounded-lg justify-center bg-gray-800 shadow-md text-white font-subtitle bold-18 py-1 px-2 cursor-pointer hover:bg-gray-900 hover:shadow-md mt-2">
